...
function createSlide(row, slideIndex, carouselId) {
  ...
  row.querySelectorAll(':scope > div').forEach((column, colIdx) => {
    if (colIdx === 0) {
      column.classList.add('carousel-video-slide-image');
      const link = column.querySelector('a[href*="youtube"]');
      const textContent = column.textContent.trim();
      const videoUrl = link ? link.href : textContent;
      const videoId = getYouTubeId(videoUrl);
      if (videoId) {
        const wrapper = document.createElement('div');
        wrapper.classList.add('carousel-video-embed');
        wrapper.innerHTML = `<iframe src="https://www.youtube.com/embed/${videoId}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy"></iframe>`;
        column.textContent = '';
        column.append(wrapper);
      }
    } else {
      column.classList.add('carousel-video-slide-content');
    }
    slide.append(column);
  });
